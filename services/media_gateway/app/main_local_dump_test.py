...
class MediaGatewaySession:
    def __init__(
        ...
        # Local dump test mode
        self.local_dump_video = (
            os.getenv("MEDIA_GATEWAY_LOCAL_DUMP_VIDEO", "false").lower() == "true"
        )
        self.local_dump_audio = (
            os.getenv("MEDIA_GATEWAY_LOCAL_DUMP_AUDIO", "false").lower() == "true"
        )
        self.local_dump_dir = os.getenv(
            "MEDIA_GATEWAY_LOCAL_DUMP_DIR", "/tmp/media_gateway_dump"
        )
        self.local_dump_frame_limit = int(
            os.getenv("MEDIA_GATEWAY_LOCAL_DUMP_FRAME_LIMIT", "300")
        )
        self.local_dump_audio_limit = int(
            os.getenv("MEDIA_GATEWAY_LOCAL_DUMP_AUDIO_LIMIT", "50")
        )
        self.local_dump_frame_index = 0
        self.local_dump_audio_index = 0
        ...
        self._status = {
            "session": {
                ...
                "last_error": None,
                "last_warning": None,
                "local_dump_video": self.local_dump_video,
                "local_dump_audio": self.local_dump_audio,
                "local_dump_dir": self.local_dump_dir,
            },
            "video": {
                "pad_seen": False,
                "branch_built": False,
    ...
    def start(self):
        ...
        if self.local_dump_video or self.local_dump_audio:
            os.makedirs(self.local_dump_dir, exist_ok=True)
    ...
    def _dump_video_frame_if_enabled(self, jpeg_bytes: bytes):
        if not self.local_dump_video:
            return

        if self.local_dump_frame_index >= self.local_dump_frame_limit:
            return

        frame_path = os.path.join(
            self.local_dump_dir,
            f"frame_{self.local_dump_frame_index:06d}.jpg",
        )
        with open(frame_path, "wb") as f:
            f.write(jpeg_bytes)

        self.local_dump_frame_index += 1
        with self._status_lock:
            self._status["video"]["dumped_frame_count"] = self.local_dump_frame_index
    ...
    def _on_video_appsink_sample(self, sink):
        ...
        try:
            ...
            self._dump_video_frame_if_enabled(jpeg_bytes)
    ...
    def _dump_audio_chunk_if_enabled(self, wav_bytes: bytes):
        if not self.local_dump_audio:
            return

        if self.local_dump_audio_index >= self.local_dump_audio_limit:
            return

        chunk_path = os.path.join(
            self.local_dump_dir,
            f"audio_{self.local_dump_audio_index:06d}.wav",
        )
        with open(chunk_path, "wb") as f:
            f.write(wav_bytes)

        self.local_dump_audio_index += 1
        with self._status_lock:
            self._status["audio"]["dumped_audio_chunk_count"] = self.local_dump_audio_index
    ...
    def _enqueue_audio_chunks_if_ready(self):
        while len(self.audio_buffer) >= self.audio_target_chunk_bytes:
            ...
            self._dump_audio_chunk_if_enabled(wav_bytes)
